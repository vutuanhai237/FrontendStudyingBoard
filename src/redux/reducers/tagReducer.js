...
export default function tagReducer(state = initialState, action) {
    switch (action.type) {
        ...
        case GET_TAG_SEARCH_RESULT_SUCCESS:
            {
                return { ...state, tagSearchResult: { isLoading: false, data: action.payload, itemCount: 30, error: '' } }
            }
        case GET_TAG_SEARCH_RESULT_FAILURE:
            {
                return { ...state, tagSearchResult: { isLoading: false, error: action.payload, data: [], itemCount: 0 } }
            }
        default:
            return state;
    }
